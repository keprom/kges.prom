<?php if(false): ?>
<h4>Данные для вычисления потерь для данной Т.У.</H4>
<?php
...
Напряжение (U)
<input name=napr value='<?php echo $point_data->napr; ?>'   /><br/>
Сопротивление (r0)
<input  name=sopr value='<?php echo $point_data->sopr; ?>' /><br/>
Длина (L)
<input  name=dlina value='<?php echo $point_data->dlina; ?>' /><br/>
Тангенс (tgf)
<input  name=tgf value='<?php echo $point_data->tgf; ?>' /><br/>
Трансфоматоры(S ном) 
<input  name=snom value='<?php echo $point_data->snom; ?>' /><br/>
Дельта Ркз уд.
<input  name=rkz value='<?php echo $point_data->rkz; ?>' /><br/>
<br><br><br>
