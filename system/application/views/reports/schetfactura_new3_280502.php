...
<FONT size=7>
 <table width=100%>
 <tr align=center>
 <td width=2100px> <b><?php echo (strlen(trim($akt_vypolnenyh_rabot))==0?"Есеп шот фактура / Счет-фактура":$akt_vypolnenyh_rabot);?> №</b> <?php echo "280502 от 30.06.2017";?>
 </td>
 <td align=left> (1)</td>
 </tr>
 <tr><td></td><td></td></tr>
 ...
 <tr>
 <td width=2100px>Жеткізушінің БСН мен мекенжайы<br />БИН и адрес поставщика  <u>  <?php echo "БИН ".$org->bin.", ".$org->address;?> Свидетельство о постановке на регистрационный учет по НДС серия 03001 № 0007114 от 17.09.2012</u></td>
 </tr>
  <tr>
 ...
 <tr>
 <td width=2100px>Тауарлар (жұмыстар, қызметтер) жеткізуге арналған шарт (келісім-шарт)<br/>Договор(контракт) на поставку товаров (работ, услуг) №<u>   <?php if (strlen($dog2)=='') {echo $firm->dogovor."   </u> от <u>    ".datetostring($firm->dogovor_date);} else echo $dog2;?></u></td>
 <td> (3)</td>
 </tr>
 <tr>
 ...
 <tr>
 <td width=2100px>Жүк жіберуші<br/> Грузоотправитель <u>филиал ТОО "Кокшетау Энерго Центр" Горэлектросети  БИК NURSKZKX КБЕ 17 БИН 091241017203 АО "Нурбанк"                                                                                                                   </u></td>
 <td> (8)</td>
 </tr>
 <tr align=right>
 ...
 <td> </td>
  ...
  <tr>
 <td width=2100px><b>Сатып алушы<br/>Покупатель</b> <u>        <?php if (strlen($edit7)==0){ echo "{$firm->name}";} else {echo $edit7;} ?>                </u></td>
 <td> (10)</td>
 </tr>
 <tr>
 <td width=2100px>Сатып алушының БСН<br/>БИН и адрес нахождения покупателя <u> <?php if (strlen($edit8)==0) { echo "БИН ".$firm->bin.", ".$firm->address;} else {echo $edit8;;}?></u></td>
 </tr>
 <tr>
...
<tr align="center">
	...
	<TD width="200px"><?php echo f_d((-1)*$s2->kvt); ?></td>
	...
	<?php
		...
		{
			echo f_d((-1)*$s2->nds_value);
			$sum_nds+=$s2->nds_value;
			$i_nds-=f_d((-1)*$s2->nds_value);
		}
		else
			echo f_d((-1)*$i_nds);
	?>
	...
	<?php 
		...
		{
			echo f_d((-1)*$s2->with_nds);
			$sum+=$s2->with_nds;
			$i_itogo-=f_d((-1)*$s2->with_nds);
		}
		else
			echo f_d((-1)*$i_itogo);
	?>
...
<td width="300px" align="right"><b><?php echo f_d((-1)*$itog->itog_tenge);?></b></td>
<td width="110px" bgColor=#c0c0c0></td>
<td width="250px" align="right"><b><?php echo f_d((-1)*$itog->itogo_nds);?></b></td>
<td width="400px" align="right"><b><?php echo f_d((-1)*$itog->itogo_with_nds);?></b></td>
<td width="110px" bgColor=#c0c0c0></td>
<td width="100px"></td>
</tr>
</table>
...
<br />
 ...
 <table width="100%">
 <tr>
 <td align="left"><b>Уполномоченный подписывать счета фактуры за руководителя согласно приказа №64 от 31.12.2015г.:<br/> <?php  echo 'Ламнова Л.П.'; ?></b></td>
 <td></td>
 ...
 <tr>
 <td align="left"><b>Уполномоченный подписывать счета фактуры за главного бухгалтера согласно приказа №64 от 31.12.2015 г.:<br/> <?php echo "Ламнова Л.П."; ?> </b></td>
 <td></td>
 <td align="right"></td>
 </tr>
 <tr>
